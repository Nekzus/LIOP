...
- **2026-05-22**: **Optimización de Seguridad y Estabilidad del Ecosistema**. 
  ...
  - **Hardening de Suite de Pruebas**: Modificado `vitest.config.ts` en el SDK TypeScript sustituyendo el pool por defecto de `worker_threads` por un pool de subprocesos aislados (`pool: "forks"`). Se limitó secuencialmente a un único fork en entornos de CI (`singleFork: !!process.env.CI`), neutralizando fugas de memoria y crashes nativos de hilos en servidores de recursos limitados (GitHub Actions). Batería completa de 286/286 tests unitarios superada con éxito (Exit code 0).
