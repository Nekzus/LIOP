...
```
Healthcare Data Enclave (Hospital Private VPC):
┌─────────────────────────────────────────────────────────────────────────────┐
│  LiopServer (Clearance Tier: 5)                                             │
│  FHIR Patient Database (HL7 v4.0.1)                                         │
│                                                                             │
│  Injected Query: "Incidence of Type 2 Diabetes among cohort aged 45-65"     │
│                                                                             │
│  Step 1: Compute Raw Count ──────────► count = 1,482                        │
│  Step 2: Add Laplace Noise (ε = 0.5) ─► noisyCount = 1,482 + Lap(1 / 0.5)  │
│                                                   = 1,484                   │
│  Step 3: Egress PII Shield ──────────► Zero PHI detected (Clean)            │
│  Step 4: Seal with ZK-Receipt ───────► Output signed under ML-KEM session   │
└──────────────────────────────────────┬──────────────────────────────────────┘
                                       │ Wire Transfer: 195 bytes (No PHI)
                                       ▼
                         External Clinical AI Agent
```
