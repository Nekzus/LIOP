---
title: "LiopClient"
description: "Enrutamiento de intenciones e inyección de lógica autónoma mediante el Logic-Injection-on-Origin Protocol"
---
...
A diferencia de los clientes REST tradicionales que solo realizan peticiones HTTP a endpoints JSON estáticos, un `LiopClient` despacha sus funciones de lógica a través de la red para ser ejecutadas de manera nativa en el servidor.
...
## Invocación de Capacidades del Servidor (Tools)
...
## Configuración de Watchdogs (Suscripciones Push) [PLANIFICADO]
...
## Validación Criptográfica (Recibos ZK)

Aunque la capa de red está cifrada usando AES-256-GCM simétrico, el `LiopClient` aplica el principio de verificación continua de la integridad computacional.
...
Debido a que el Servidor impone un sandbox `WASI` estricto y utiliza *Guardian AST* para inspeccionar el payload `.wasm` antes de la ejecución, tu cliente debe estar preparado para manejar rechazos del sandbox.
