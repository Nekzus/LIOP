...
Al crear un `LiopClient`, el segundo argumento configura su presencia en la red:
...
A diferencia de MCP, que requiere una tubería stdio directa o una URL de SSE preconfigurada, LIOP utiliza ruteo peer-to-peer descentralizado. 
Tú proporcionas un **Peer ID** (la identidad única Ed25519 del Servidor al que quieres llegar) y el SDK se encarga de atravesar NATs automáticamente.
...
El verdadero poder de la Lógica-Inyectada-en-el-Origen es la habilidad de generar **Watchdogs**. En lugar de consultar manualmente `client.callTool()` cada 5 segundos, cargas un módulo de monitoreo persistente.
...
```
*Nota: El adaptador `LiopMcpBridge` ejecuta automáticamente esta validación de forma nativa en favor de clientes MCP legacy (como Claude o Cursor), interceptando cualquier JSON alucinado automáticamente.*
