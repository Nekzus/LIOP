---
title: "LiopServer"
description: "Construcción de Nodos de Datos autónomos y migración de herramientas MCP heredadas"
---
...
Cuando un Nodo de Datos recibe un payload `.wasm` o `.js`, el módulo **`GuardianTS`** somete su Árbol de Sintaxis Abstracta (AST) a una inspección heurística previa a la ejecución para verificar las restricciones de seguridad.

Antes de que cualquier módulo WebAssembly sea instanciado o ejecutado, `GuardianTS` escanea todas sus importaciones. Rechaza estrictamente cualquier payload que intente enlazarse a APIs no autorizadas del host. Solamente las APIs estándar de WASI (`wasi_snapshot_preview1`) y las funciones nativas en sandbox de `LIOP` son permitidas, previniendo escapes del sandbox.

Los resultados de la evaluación de AST se almacenan en caché en memoria `O(1)` para optimizar las ejecuciones subsiguientes del mismo payload.

Si despliegas actualizaciones de seguridad en el host o requieres invalidar la memoria del nodo, puedes purgar la caché del AST manualmente:

```typescript
server.clearAstCache();
```

## Conexión con Servidores MCP Heredados (Legacy)
...
El paquete `@nekzus/liop` incluye el módulo `LiopMcpBridge`.

Este adaptador intercepta las peticiones binarias entrantes de la malla LIOP, las traduce al estándar JSON-RPC 2.0 localmente, las reenvía a tu servidor MCP sin modificar vía stdio o SSE, y empaqueta la respuesta de vuelta hacia la red P2P.
...
El servicio MCP queda disponible para agentes en la malla sin requerir modificaciones en el código original.
...
Una vez que tus herramientas, watchdogs y puentes están configurados, inicias el Nodo de Datos vinculándolo a la red:
