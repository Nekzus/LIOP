...
<picture>
  <source srcset="/images/animated-log-audit-interceptors-dark.svg" media="(prefers-color-scheme: dark)" />
  <img src="/images/animated-log-audit-interceptors-light.svg" alt="Arquitectura de Interceptores del Protocolo LIOP" />
</picture>
...
Si la implementación de un interceptor invoca código que dispara `LiopLogger` (directamente o mediante dependencias), un bucle reentrante infinito provocaría un desbordamiento de pila (*stack overflow*). `LiopLogger` mantiene una bandera interna de reentrancia (`_isIntercepting`) que suprime el despacho recursivo del interceptor dentro del mismo marco de llamada, preservando la salida estándar a `stderr`.
