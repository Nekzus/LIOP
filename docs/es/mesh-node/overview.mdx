...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                            liop-node (Core en Rust)                         │
├───────────────────────────────┬─────────────────────────────────────────────┤
│         Capa de Red           │               Capa de Ejecución             │
│  Tonic gRPC (h2 / QUIC)       │  Wasmtime v29.0 (WASI Preview 1)            │
│  rust-libp2p (Kademlia DHT)   │  Medición de Instrucciones (`set_fuel`)     │
│  Protocolo Noise (Ed25519)    │  Aislamiento Lineal y Trampas de Memoria    │
│  Post-Cuántica (ML-KEM-768)   │  Ganchos de Recibos ZK (HMAC-SHA256)        │
└───────────────────────────────┴─────────────────────────────────────────────┘
```
...
1. **Tonic (gRPC sobre HTTP/2):**
   Tonic gestiona el empaquetado de mensajes, el control de flujo y la contrapresión de forma asíncrona sobre el runtime multihilo de `tokio`. Todas las cargas se serializan en Protobuf binario, suprimiendo la sobrecarga de codificación JSON.
2. **rust-libp2p (Noise y Kademlia):**
   Los nodos autentican su identidad criptográfica Ed25519 mediante el **Noise Protocol Framework** (patrón de negociación `XX`). Los pares descubren capacidades y contenidos a través del identificador canónico de Kademlia DHT (`/ipfs/kad/1.0.0`), eliminando la dependencia de servidores DNS o autoridades de certificación centralizadas.
