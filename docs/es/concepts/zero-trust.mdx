...
El Logic-Injection-on-Origin Protocol (LIOP) asume que la red es hostil por defecto, adoptando una **arquitectura Zero-Trust integral**. Cada capa integra protecciones criptográficas y de aislamiento avanzadas.

## 1. Cifrado de Transporte (P2P & PQC)

LIOP utiliza el **Noise Protocol Framework** para el establecimiento de conexiones P2P, eliminando la dependencia de Autoridades de Certificación (CAs) centralizadas y reforzado con Criptografía Post-Cuántica:
...
**Validación Estructural ZK**: Esta validación (`verifyZkReceipt`) es ejecutada de forma nativa tanto por el `LiopClient` como por el adaptador `LiopMcpBridge`. Estos componentes actúan como filtros de verificación criptográfica, comprobando las firmas e impidiendo que datos adulterados u orígenes comprometidos sean entregados a la IA.
