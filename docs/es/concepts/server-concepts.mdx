...
Para datasets por debajo del `smallDatasetThreshold` (por defecto: 50 registros), el servidor aplica automáticamente ruido Laplace calibrado a todas las salidas numéricas. Configure el perfil de privacidad por herramienta:
...
server.registerTool("analyze", handler, {
  ...
  queryBudgetPerField: 5, // Máximo de consultas por campo
});
```
