...
Un Agente LIOP interactuando con un Servidor LIOP inyecta lógica ejecutable (en forma de módulos `.wasm` compactos o algoritmos transpilados dinámicamente). El servidor ejecuta de forma segura esta lógica dentro de un estricto sandbox y retorna únicamente resultados matemáticos agregados o listas filtradas, impidiendo la exfiltración involuntaria de PII (Información de Identificación Personal) derivada de extracciones masivas de contexto.
...
- LIOP opera sin intermediarios ni servidores proxy centralizados.
...
## 4. Arquitectura de Seguridad Criptográfica
...
## 5. Núcleo de Ejecución y Aislamiento
...
### 5.2 Guardián AST Estático
- Previo a la ejecución, LIOP analiza el Árbol de Sintaxis Abstracta (AST) del payload. Descarta módulos que intenten importar funciones fuera de la allowlist estricta de 14 funciones WASI, soportando sentencias `return` en el nivel superior para módulos dinámicos.
...
## 6. Remediación Autónoma de Protocolo
LIOP implementa un middleware de auto-instrucción integrado. Si un Agente intenta interactuar mediante solicitudes JSON-RPC sobre adaptadores MCP tradicionales violando el paradigma Logic-Injection-on-Origin (ej. solicitar extracción de datos crudos), LIOP intercepta la petición, la bloquea y devuelve un prompt estructurado que permite al agente reformular su petición como lógica inyectada.
