...
<Frame caption="Contraste: Extracción de Contexto frente a Logic-Injection-on-Origin (LIOP)">
  <img className="block dark:hidden w-full" src="/images/logic-vs-pull-light.svg" alt="Extracción de Contexto frente a Logic-Injection-on-Origin" />
  <img className="hidden dark:block w-full" src="/images/logic-vs-pull-dark.svg" alt="Extracción de Contexto frente a Logic-Injection-on-Origin" />
</Frame>
...
Consulta la [Especificación del Protocolo](/es/concepts/specification), evalúa el [Playground Web y Studio](/es/typescript-sdk/playground), o despliega un nodo enclave mediante el SDK TypeScript ejecutando `npm install @nekzus/liop`. Consulta la [Guía de Inicio](/es/getting-started/quickstart).
