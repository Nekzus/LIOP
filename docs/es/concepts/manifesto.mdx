...
Consulta la [Especificación del Protocolo](/es/concepts/specification), evalúa el [Playground Interactivo](http://localhost:16000), o despliega un nodo enclave mediante el SDK TypeScript ejecutando `npm install @nekzus/liop`. Consulta la [Guía de Inicio](/es/getting-started/quickstart).
