...
When a Data Node receives a `.wasm` or `.js` payload, the **`GuardianTS`** module performs a pre-execution heuristic inspection on its Abstract Syntax Tree (AST) to verify security constraints.

Before any WebAssembly module is instantiated or executed, `GuardianTS` scans all imports. It rejects payloads attempting to bind to unauthorized host APIs. Only standard WASI APIs (`wasi_snapshot_preview1`) and native `LIOP` sandboxed functions are permitted, preventing sandbox escapes.

AST evaluation results are cached in memory `O(1)` to optimize subsequent executions of the same payload.

If you are deploying security updates on the host or need to invalidate the execution memory, you can purge the AST cache manually:
