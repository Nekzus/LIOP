...
* **Bootstrap Monte Carlo Scaling**: Sustained statistical inference workloads (30 to 40 bootstrap iterations with variance, covariance, and Pearson correlation) cause deterministic WASI fuel consumption to scale proportionally from 300 to 400 units to 700 to 900 units (+75% to +125%), captured live by the `liop_fuel_consumed_total` histogram.
...
* **Hardware Stability**: Verified under WSL2 (`memory=6GB`, `processors=4`) with 0% swap usage and aggregate CPU at ~14% of 400% available, with zero thermal saturation or pipe lockups.
