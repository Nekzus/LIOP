...
# Runs the 11 automated test suites (59 tests) inside the audit runner container
pnpm audit:prod:run
```
...
## Automated Verification Suites (Suites 00 to 10)

The audit harness executes **11 comprehensive Vitest suites** (59 individual tests) inside the `liop-audit-runner` container:
...
11. **`plug-and-play`**: End-to-end "plug-and-play" validation confirming that the SDK boots, discovers peers, executes logic injections, and returns cryptographically attested results without any manual configuration.
