...
The LIOP TypeScript SDK enforces a **defense-in-depth** security model composed of four independent layers. Each layer addresses a distinct threat surface — from static code analysis to post-execution data filtering — ensuring that no single point of failure can compromise the system.
...
Twelve attack vectors are neutralized by setting each to `undefined` and sealing them as non-writable:
...
The PII Scanner intercepts all sandbox output **before** results are returned to the caller. It applies a combination of regex patterns and algorithmic validators to detect and block sensitive data.
...
- **Forbidden Key Lookup**: O(1) blocking via `Set<string>.has()` for field names like `ssn`, `email`, `password`, `privateKey`
...
2. **Journal Assembly**: A JSON object containing `image_id`, `output_hash` (SHA-256 of the result), `fuel` consumed, and `timestamp`
3. **HMAC Seal**: `crypto.createHmac("sha256", sessionSecret).update(journal).digest()` — the session secret is derived from the **ML-KEM-768 (Kyber) key exchange**
4. **Verification**: `crypto.timingSafeEqual(seal, expectedSeal)` — constant-time comparison prevents timing side-channel attacks
