...
Every `@nekzus/liop` server instance exposes a native **Prometheus text-format** metrics endpoint at `GET /metrics`. No sidecar, agent, or external dependency is required, as the metrics registry is built into the SDK core and refreshes process-level gauges synchronously on each scrape request.
...
| `liop_egress_blocks_total` | Counter | N/A | Outputs blocked by Egress PII Shield |
...
| `liop_tokens_input_total` | Counter | N/A | BPE input tokens processed (`o200k_base`) |
| `liop_tokens_output_total` | Counter | N/A | BPE output tokens emitted |
| `liop_tokens_saved_total` | Counter | N/A | Estimated tokens saved vs extracting the origin dataset (`originDatasetTokens - outputTokens`) |
...
| Metric | Type | Labels | Description |
|---|---|---|---|
| `liop_mesh_peers_connected` | Gauge | N/A | Active P2P peer connections (Kademlia DHT) |
| `liop_manifest_cache_size` | Gauge | N/A | Verified remote tool manifests in cache |
| `liop_node_health_status` | Gauge | N/A | Operational health: `1` = healthy, `0` = degraded |
| `liop_process_uptime_seconds` | Gauge | N/A | Node process uptime in seconds |
| `liop_process_memory_rss_bytes` | Gauge | N/A | Resident Set Size (physical memory) |
| `liop_process_memory_heap_used_bytes` | Gauge | N/A | V8 heap memory in use |
| `liop_process_memory_heap_total_bytes` | Gauge | N/A | V8 total allocated heap |
| `liop_process_memory_external_bytes` | Gauge | N/A | Memory bound to C++ objects and ArrayBuffers |

<Note>
  Gauges like `liop_mesh_peers_connected` and `liop_manifest_cache_size` are sampled **synchronously inside the `GET /metrics` handler**, ensuring Prometheus always receives the instantaneous runtime state rather than a stale cached snapshot.
</Note>
...
| `NODE_OPTIONS` | N/A | V8 heap configuration (recommended: `--max-old-space-size=2048`) |
