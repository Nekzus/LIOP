...
When creating a `LiopClient`, the second argument configures its network footprint:
...
Unlike MCP which requires a direct stdio pipe or a hardcoded SSE URL, LIOP uses decentralized peer-to-peer routing. 
...
The true power of Logic-Injection-on-Origin is the ability to spawn **Watchdogs**. Instead of manually polling `client.callTool()` every 5 seconds, you upload a persistent watcher module.
...
```
*Note: The `LiopMcpBridge` automatically runs this capability natively on behalf of legacy MCP clients, intercepting any hallucinated JSONs automatically.*
