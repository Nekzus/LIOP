...
- **Timing Side-Channel Defense:** The `Date` object is poisoned within the sandbox to prevent injected logic from measuring execution time differences, which could be used to infer dataset size or internal execution patterns.
...
- **Prototype Pollution Defense:** Six core JavaScript prototypes (`Object`, `Array`, `String`, `Number`, `Boolean`, `Function`) are frozen via `Object.freeze()` inside the sandbox IIFE before user code executes, preventing injected logic from overriding built-in methods.
- **Non-Blocking Pool:** The Sandbox execution is inherently computationally heavy. To prevent Node.js servers from freezing, the `@nekzus/liop` isolates this entire lifecycle inside native `worker_threads` (via `piscina`), achieving parallel throughput identical to the native Mesh-Node.
