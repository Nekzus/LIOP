...
For datasets below the `smallDatasetThreshold` (default: 50 records), the server automatically applies calibrated Laplace noise to all numeric outputs. Configure the privacy profile per tool:

```typescript
server.registerTool("analyze", handler, {
  dpEpsilon: 2.0,        // Privacy budget (higher = less noise, more utility)
  dpSensitivity: 1000.0, // Max change per record for SUM fields
  queryBudgetPerField: 5, // Max queries before field exhaustion
});
```
