...
async function main() {
    ...
    if (bootstrapNodes.length === 0) {
        bootstrapNodes.push("/ip4/127.0.0.1/tcp/4001/p2p/12D3KooWEZ3Jy2tu65g5ZGVq1t3TrzKnrqhbvfCmMgdA9cNAEiKY");
    }
    ...
    const router = new NmpMcpRouter(nmpServer, meshNode, 50051, [
        {
            ...
            description: "Processes sensitive medical data blinded securely via NMP Zero-Trust WASM (Hosted on The Vault)",
            inputSchema: {
                type: "object",
                properties: { patientId: { type: "string", description: "The ID of the patient (e.g., NMP-99)" } },
                required: ["patientId"]
            }
        },
        {
            name: "CheckBalance",
            description: "Securely checks the bank account balance without exposing PII (Hosted on The Bank)",
            inputSchema: {
                type: "object",
        ...
        {
            name: "GetStockPrice",
            description: "Fetches the real-time stock price for a given ticker symbol (Hosted on The Oracle)",
            inputSchema: {
                type: "object",
    ...
    console.error(`[NMP-Agent] 🌐 P2P Mesh: Joined (${bootstrapNodes.length} bootstraps)`);
