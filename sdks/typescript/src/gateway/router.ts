...
export class NmpMcpRouter {
    ...
    public async dispatch(request: any): Promise<any> {
        ...
        switch (method) {
            ...
            case 'resources/list':
                return { jsonrpc: '2.0', id, result: { resources: this.nmpServer.listResources() } };
            case 'resources/read': {
                if (!params?.uri) return { jsonrpc: '2.0', id, error: { code: -32602, message: 'Missing resource uri' } };
                try {
            ...
            case 'prompts/list':
                return { jsonrpc: '2.0', id, result: { prompts: this.nmpServer.listPrompts() } };
            default:
    ...
    private async transcodeMcpToNmp(id: any, params: any): Promise<any> {
        ...
        const fallbackClient = new nmpV1.NeuralMesh(`127.0.0.1:${fallbackPort}`, createChannelCredentials());
        return this.performTranscoding(id, fallbackClient, toolName, params);
    }

    private async routeToRemoteProvider(id: any, toolName: string, peerId: string, params: any): Promise<any> {
        if (!this.meshNode) return { jsonrpc: '2.0', id, error: { code: -32603, message: 'Mesh Node inactive' } };

        const addrs = await this.meshNode.resolvePeer(peerId);
        if (addrs.length === 0) return { jsonrpc: '2.0', id, error: { code: -32002, message: 'Remote Peer unreachable' } };
        ...
        if (!targetAddr) return { jsonrpc: '2.0', id, error: { code: -32002, message: 'Invalid Remote Multiaddr' } };

        const remoteClient = new nmpV1.NeuralMesh(targetAddr, createChannelCredentials());
        return this.performTranscoding(id, remoteClient, toolName, params);
    }

    private async performTranscoding(id: any, client: any, toolName: string, params: any): Promise<any> {
        return new Promise((resolve) => {
            ...
            client.negotiateIntent({
                ...
                if (err || !response.accepted) {
                    return resolve({ jsonrpc: '2.0', id, error: { code: -32001, message: 'PQC Handshake Failed' } });
                }

                const { ciphertext, sharedSecret } = Kyber768Wrapper.encapsulateAsymmetric(response.kyber_public_key);
                ...
                let resultBody = '';
                call.on('data', (grpcRes: LogicResponse) => { resultBody += grpcRes.semantic_evidence; });
                call.on('end', () => {
                    try {
                        const parsedResult = JSON.parse(resultBody);
                        resolve({ jsonrpc: '2.0', id, result: parsedResult });
                    } catch (e) {
                        resolve({ jsonrpc: '2.0', id, result: { content: [{ type: 'text', text: resultBody }] } });
                    }
                });
                call.on('error', (e: any) => resolve({ jsonrpc: '2.0', id, error: { code: -32603, message: `NMP Error: ${e.message}` } }));
            });
        });
    }

    private encryptWithNonce(payload: Buffer, key: Uint8Array, nonce: Buffer): Buffer {
