...
export class NmpHybridGateway {
    ...
    constructor(private nmpServer: NmpServer, config: { meshConfig?: MeshNodeConfig; rpcPort?: number; tls?: NmpTlsOptions } = {}) {
        ...
        this.netServer = net.createServer((socket) => {
            ...
            socket.on('error', (err) => console.error(`[NMP-Hybrid] Socket Error: ${err.message}`));
        });
    }
    ...
    private handleGrpcStream(stream: http2.ServerHttp2Stream) {
        stream.on('data', (chunk: any) => {
            ...
            if (decoded.data) console.error(`[NMP-Hybrid] Native gRPC Proxy passing ${decoded.data.length} bytes`);
        });
        stream.respond({ ':status': 200, 'content-type': 'application/grpc' });
        stream.end();
    }

    private handleMcpH2Stream(stream: http2.ServerHttp2Stream, _headers: http2.IncomingHttpHeaders) {
        let body = '';
        ...
        stream.on('end', async () => {
            ...
            } catch (e) { stream.respond({ ':status': 400 }); stream.end(); }
        });
    }
    ...
    public async listen(port: number, host: string = '::'): Promise<void> {
        if (this.meshNode) {
            ...
            for (const tool of tools) {
                ...
                console.error(`[NMP-Hybrid] 📡 Announced local tool to Mesh: ${tool.name}`);
            }
        }
