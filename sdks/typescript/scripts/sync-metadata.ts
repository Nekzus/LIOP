...
async function syncMetadata() {
    ...
    updateJson(path.join(rootDir, "docs/docs.json"), (config) => {
        ...
        if (config.navbar && config.navbar.links) {
        ...
        if (config.navigation && config.navigation.languages) {
            for (const lang of config.navigation.languages) {
                for (const tab of lang.tabs) {
                    if (tab.href && tab.href.includes("github.com")) {
                        tab.href = tab.href
        ...
        if (config.footer && config.footer.socials) {
    ...
    const patterns = [
        { from: /github\.com\/Nekzus-Solutions\/LIOP-v1\.0-alpha/g, to: `github.com/${newRepoFull}` },
