...
async function main() {
    ...
    theVaultServer.tool(
        ...
        async (args) => {
            return {
                content: [{ type: "text", text: "Computation successful. Records processed: 5" }]
            };
        }
    );
    ...
    const hybridGateway = new NmpHybridGateway(theVaultServer, { rpcPort: RPC_PORT });
    await hybridGateway.listen(3000, '0.0.0.0');
    console.log(`[VAULT-SERVER] NMP Unified Mock Server READY (forbiddenKeys: id, name).`);
}
