...
async function main() {
    ...
    try {
        ...
        console.log(`[The Sentinel] 📎 Read Vault Bootstrap Address: ${vaultMultiaddr}`);
    } catch (e) {
    ...
    console.log(`[The Sentinel] 🌐 Joining Zero-Trust Mesh targeting Vault Bootstrap...`);
    
    // 3. Start proxy listening on HTTP port 3000
    await gateway.listen(3000);
    
    console.log(`[The Sentinel] 🚀 Transformer gateway running on http://localhost:3000/mcp`);
    console.log(`[The Sentinel] Ready to transcode JSON-RPC (MCP) to PQC gRPC (NMP).`);
