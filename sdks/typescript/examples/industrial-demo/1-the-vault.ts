...
async function main() {
    ...
    server.tool(
        ...
        async (args) => {
            ...
            console.log(`[The Vault] -> Processing DB Record for Patient: ${args.patientId}`);
            return {
                content: [{
                    type: "text",
                    text: JSON.stringify({
    ...
    const p2pAddr = multiaddrs.find(a => a.includes('/ip4/127.0.0.1/tcp/')) || multiaddrs[0];
    await fs.writeFile("vault.multiaddr", p2pAddr);

    console.log(`[The Vault] 🚀 Ready and protecting Data. Waiting for Sentinel requests...`);
