...
const CANDIDATE_PROFILES: KnownNodeProfile[] = [
	{
		id: "vault",
	...
	},
	{
		...
		defaultTools: ["Analyze_HFT_Market_Data"],
	},
	{
		id: "relay",
...
export class NetworkDiscoveryEngine {
	...
	public async scanNetwork(host = "127.0.0.1"): Promise<ScannedTargetNode[]> {
		const results: ScannedTargetNode[] = [];

		const probePromises = CANDIDATE_PROFILES.map(async (profile) => {
			...
			if (probe) {
				...
				const cleanName = rawName.startsWith("PRODUCTION-")
					? rawName.replace("PRODUCTION-", "").replace(/-/g, " ").toUpperCase()
					: rawName;
	...
	public async resolveNodeForGrpcTarget(
		...
		// Match candidate profile by grpcPort
		const matchedProfile = CANDIDATE_PROFILES.find(
		...
		// Fallback for custom gRPC address
